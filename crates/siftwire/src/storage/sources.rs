...
const SOURCE_COLUMNS: &str = "key, label, kind, url, repo, section, threshold, enabled, \
    url_canonicalization, outlet_extraction, dedup_group, priority_rank, \
    schedule_format, schedule_filter, api_key";

impl Store {
    /// Lists sources ordered by key, rejecting unsupported stored configuration.
    ///
    /// # Errors
    ///
    /// Returns an error when `SQLite` cannot read a row or its configuration is unsupported.
    pub fn list_sources(&self, enabled_only: bool) -> Result<Vec<Source>> {
        ...
        rows.map(|row| {
            ...
            normalize_source(source).with_context(|| format!("unsupported stored source {key:?}"))
        })
        .collect()
    }
...
fn source_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Source> {
    ...
    Ok(Source {
        ...
        schedule_format: row.get(12)?,
        schedule_filter: row.get(13)?,
        api_key: row.get(14)?,
    })
}
