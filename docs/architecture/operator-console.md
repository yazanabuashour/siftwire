...
The operator-tooling plan put CLI commands first, then a read-only viewer, then
a configuration dashboard when a concrete task required writes. The concrete
task now exists: the operator wants to view and edit sources, inspect deliveries
including past ones, and review candidate selection from a browser. Prooflane already ships a loopback
...
Rust server and two speculative consumers. No existing caller needs that shared
layer.
...
`delete_source`, `replace_outlet_policies`, and `set_brief_options`. The console
has no SQLite dependency and duplicates no storage code. This keeps
...
already owns those validated actions. The console does not expose run execution.
A dashboard button for `run_brief` could advance latest-seen state on an accidental
click; retrying the run is not safe.
